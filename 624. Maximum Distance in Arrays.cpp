...
to be their absolute difference |a - b|. Return the maximum distance.
...
public:
  int maxDistance(vector<vector<int>> &arrays) {
    int minEl = arrays[0][0], maxEl = arrays[0][arrays[0].size() - 1], res = 0;
    for (int i = 1; i < arrays.size(); i++) {
      int currMin = arrays[i][0];
      int currMax = arrays[i][arrays[i].size() - 1];
      res = max(res, max(abs(maxEl - currMin), abs(currMax - minEl)));
      minEl = min(minEl, currMin);
