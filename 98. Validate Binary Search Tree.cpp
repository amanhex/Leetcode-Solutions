...
class Solution {
private:
  bool f(TreeNode *root, long long lowerBound, long long upperBound) {
    if (!root)
      return true;
    if (root->val <= lowerBound || root->val >= upperBound)
      return false;
    return (f(root->left, lowerBound, root->val) &&
            f(root->right, root->val, upperBound));
  }

public:
  bool isValidBST(TreeNode *root) { return f(root, LLONG_MIN, LLONG_MAX); }
};
